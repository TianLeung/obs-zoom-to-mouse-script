...
function script_properties()
    ...
    obs.obs_property_set_long_description(refresh_sources,
        "点击重新填充缩放源下拉菜单，加载可用来源")

    -- Add the rest of the settings UI
    local zoom = obs.obs_properties_add_float(props, "zoom_value", "缩放比例", 1, 5, 0.5)
    ...
    obs.obs_property_set_long_description(follow,
        "启用后，鼠标追踪功能将在放大视图时自动启动，无需等待追踪切换热键触发")

    local follow_outside_bounds = obs.obs_properties_add_bool(props, "follow_outside_bounds", "跟踪到范围之外 ")
    ...
    local follow_border = obs.obs_properties_add_int_slider(props, "follow_border", "追随边界", 0, 50, 1)
    local safezone_sense = obs.obs_properties_add_int_slider(props,
        "follow_safezone_sensitivity", "锁定灵敏度", 1, 20, 1)
    local follow_auto_lock = obs.obs_properties_add_bool(props, "follow_auto_lock", "方向自动锁 ")
    obs.obs_property_set_long_description(follow_auto_lock,
        "启用后，将鼠标移至缩放源边缘即可开始追踪,\n" ..
        "但向中心回移会停止追踪，效果类似于即时战略游戏中平移摄像机")

    local allow_all = obs.obs_properties_add_bool(props, "allow_all_sources", "允许任何缩放源 ")
    obs.obs_property_set_long_description(allow_all, "启用该选项后，可将任意信号源选为变焦信号源\n" ..
        "对于非显示捕获源，您必须手动设置源位置")

    local override = obs.obs_properties_add_bool(props, "use_monitor_override", "设置手动信号源位置 ")
    obs.obs_property_set_long_description(override,
        "启用后，将使用指定的尺寸/位置设置应用于缩放源，而非自动计算得出的参数。")
    ...
    local debug = obs.obs_properties_add_bool(props, "debug_logs", "启用调试日志记录 ")
    obs.obs_property_set_long_description(debug,
        "启用后，该脚本将向脚本日志输出诊断信息（适用于调试/排查GitHub问题）")
...
function script_load(settings)
    ...
    -- Add our hotkey
    hotkey_zoom_id = obs.obs_hotkey_register_frontend("toggle_zoom_hotkey", "切换鼠标跟随缩放",
        on_toggle_zoom)

    hotkey_follow_id = obs.obs_hotkey_register_frontend("toggle_follow_hotkey", "切换鼠标跟随缩放 (锁定中心)",
        on_toggle_follow)
